...
public class MacOsScreenCaptureService : IScreenCaptureService
{
    ...
    public Task<List<CaptureTarget>> GetAvailableTargetsAsync()
    {
        ...
        _logger.LogInformation("MacOsScreenCapture: Found {Count} capture targets", targets.Count);
    ...
    public Task StartAsync(CaptureTarget target, Action<byte[], int, int> onFrame)
    {
        ...
        {
            _logger.LogError("MacOsScreenCapture: Failed to create native capture");
            return Task.CompletedTask;
        }
        ...
        if (status != 0)
        {
            _logger.LogError("MacOsScreenCapture: Failed to start capture, status={Status}", status);
        ...
        _isCapturing = true;
        _logger.LogInformation("MacOsScreenCapture: Started capturing target {Id}", target.Id);
        return Task.CompletedTask;
    }
    ...
    public Task StopAsync()
    {
        ...
        _logger.LogInformation("MacOsScreenCapture: Stopped");
        return Task.CompletedTask;
    }
        ...
        Action<byte[], long, bool> onNal, Action<byte[], int, int> onPreview)
    {
        ...
        if (_capture == IntPtr.Zero)
        {
            _logger.LogError("MacOsScreenCapture: Failed to create fused capture");
            return Task.CompletedTask;
        }
        ...
        if (status != 0)
        {
            _logger.LogError("MacOsScreenCapture: Failed to start fused capture, status={Status}", status);
            NativeVideoInterop.fennec_capture_destroy(_capture);
            _capture = IntPtr.Zero;
            return Task.CompletedTask;
        }
        ...
        _isCapturing = true;
        _logger.LogInformation("MacOsScreenCapture: Started fused capture+encode {W}x{H} {Bitrate}Kbps {Fps}fps",
            maxW, maxH, bitrateKbps, fps);
        return Task.CompletedTask;
    }

    public void UpdateFusedBitrate(int kbps)
    {
        if (_capture == IntPtr.Zero) return;
        var result = NativeVideoInterop.fennec_capture_update_bitrate(_capture, kbps);
        if (result != 0)
            _logger.LogWarning("MacOsScreenCapture: Failed to update fused bitrate to {Kbps}", kbps);
        else
            _logger.LogInformation("MacOsScreenCapture: Fused bitrate updated to {Kbps}Kbps", kbps);
    }

    public void UpdateFusedFps(int fps)
    {
        if (_capture == IntPtr.Zero) return;
        var result = NativeVideoInterop.fennec_capture_update_fps(_capture, fps);
        if (result != 0)
