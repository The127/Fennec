...
public class FederationSigningHandler(IClockService clockService, IKeyService keyService, IOptions<FennecSettings> fennecOptions) : DelegatingHandler
{
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        request.Headers.Add("X-Instance", fennecOptions.Value.IssuerUrl);
        var timestamp = clockService.GetCurrentInstant().ToUnixTimeSeconds();
